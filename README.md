...
6) Install Clover v5122 onto USB. IF you are going to install Big Sur you must use CLover v5130 or greater and its a completly different configuration I have done and now included, but you will need to read up on it if you plan on going forward Clover. A Post install note is that it will only boot preboot so add preboot into the "default boot volume" field under boot section, but with Big Sur it may be better to stick with OpenCore.
...
17) You can install Gatekeeper from Hackentool but in Big Sur the volume is read only only the Data Volume is read/write.
...
20) you have a 100% working MacOS Clone that in my opinions still runs better than any orignal MacBookPro11.2 with Dual Graphics.
21) GPU works natively.
22) I have included several useful Apps you can use and Clover Configurator a must for now, also included Plist Editor Pro will only work after you run the gatekeeper command then manually install it that comes with plist Editor Pro.  I also included Ioregistry Explorer used to determine what is working and what is not but everything is working.  I have 5 of these Asus G750JX laptops and they all run Catalina 10.15.7 flawlessly, and now running Big Sur 11.2.1 perfectly.
